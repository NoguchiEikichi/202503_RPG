...
public class FadeManager : Singleton<FadeManager>
{
	...
	//IndexNumberで画面遷移
	public void LoadSceneIndex_Battle(int sceneIndex, float interval)
	{
		StartCoroutine(TransSceneIndex_Battle(sceneIndex, interval));
	}
	...
	//IndexNumberのシーン遷移用コルーチン
	private IEnumerator TransSceneIndex_Battle(int sceneIndex, float interval)
	{
		//音量もフェードアウト
		StartCoroutine(SoundManager.Instance.FadeOut(interval * 0.9f));
